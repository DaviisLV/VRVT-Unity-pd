...
public class SPARTA_PLANE_SCRIPT : MonoBehaviour {
   // private GameObject plane_camera_parent;
...
//ROLL AND PITCH
	...
	//	verticalaxis_pos = Input.GetAxis ("Vertical");//reference
		...
		roll_speed = horizontalaxis_pos * 0.5f;//this controls the plane rotation ingame
		pitch_speed = verticalaxis_pos * 0.5f;//
...
//THIS IS TO CONTROL PLANE SPEED
		forward_speed = -0.2f;//+= -1 * Time.deltaTime;
		...
		// 	forward_speed = 0.0f;
		// }
		// if ((Input.GetKey(KeyCode.B)) && (forward_speed < 0.08f)) {//backward or brake
			
		// 	forward_constant += 0.0005f;
		// 	is_accelerating = false;
		// }
		
		// if ((Input.GetKey(KeyCode.Space)) && (forward_speed > max_speed)) {//forward
